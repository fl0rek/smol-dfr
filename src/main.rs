...
impl LibinputInterface for Interface {
    fn open_restricted(&mut self, path: &Path, flags: i32) -> Result<OwnedFd, i32> {
        ...
        OpenOptions::new()
            ...
            .open(path).map(|f| f.into()).map_err(|e| e.raw_os_error().unwrap())
    }
    fn close_restricted(&mut self, fd: OwnedFd) { _ = File::from(fd); }
}

fn emit<F: AsRawFd>(uinput: &mut UInputHandle<F>, ty: EventKind, code: u16, value: i32) {
    uinput.write(&[input_event {
        value, type_: ty as u16, code, time: timeval { tv_sec: 0, tv_usec: 0 },
    }]).unwrap();
}

fn toggle_keys<F: AsRawFd>(uinput: &mut UInputHandle<F>, codes: &[Key], value: i32) {
    if codes.is_empty() { return; }
    for kc in codes { emit(uinput, EventKind::Key, *kc as u16, value); }
...
fn main() {
    ...
    for byte in crash { for i in 0..8 {
        let c = if ((byte >> i) & 1) == 0 { 0xFF } else { 0x0 };
        map.as_mut()[wptr..wptr+4].fill(c); wptr += 4;
    }}
    drop(map);
    drm.dirty(&[ClipRect::new(0, 0, height, width)]).unwrap();
    let mut ss = SigSet::empty(); ss.add(Signal::SIGTERM); ss.wait().unwrap();
}
...
fn real_main(drm: &mut DrmBackend) {
    ...
    let drop_username = match &session_user {
        Some(u) => { eprintln!("Detected session user: {} (uid={})", u.username, u.uid); u.username.as_str() }
        None => { eprintln!("Warning: no graphical session, falling back to nobody"); "nobody" }
    };
    PrivDrop::default().user(drop_username).group_list(&["input", "video"])
        .include_default_supplementary_groups().apply()
    ...
    let udev_monitor = MonitorBuilder::new().unwrap().match_subsystem("power_supply").unwrap().listen().unwrap();
    ...
    uinput.dev_setup(&uinput_setup {
        ...
        ff_effects_max: 0, name: dev_name_c,
    }).unwrap();
    ...
    loop {
        ...
        if now_i.duration_since(last_blink).as_millis() >= 500 {
            blink_on = !blink_on; last_blink = now_i; needs_redraw = true;
        }
        timeout = min(timeout, (500 - now_i.duration_since(last_blink).as_millis() as i32).max(50));

        if let Some(deadline) = battery_time_until {
            if now_i >= deadline { battery_time_until = None; needs_redraw = true; }
        ...
        // Input
        if let Err(e) = input_tb.dispatch() { eprintln!("Warning: touchbar dispatch: {e}"); }
        if let Err(e) = input_main.dispatch() { eprintln!("Warning: main dispatch: {e}"); }
        for event in &mut input_tb.clone().chain(input_main.clone()) {
            backlight.process_event(&event);
            if backlight.take_lid_opened() { needs_redraw = true; }
            match event {
                Event::Device(DeviceEvent::Added(evt)) => {
                    if evt.device().name().contains(" Touch Bar") { digitizer = Some(evt.device()); }
                }
...
fn dispatch_message<F: AsRawFd>(
    ...
    match msg {
        ...
        Message::VolumeUpRelease => { toggle_keys(uinput, &[Key::VolumeUp], 0); *redraw = true; }
    }
}
